...
def plot_steps_from_saved(results, dims=None, data_pts=None):
    ...
    return axs
def run_plotting_script(data_file, sample_file, results_file, results_shape, days,
...
def main():
    run_plotting_script(data_file, sample_file, results_file, results_shape, days,
                        true_period=None, name_str="test", nmodels=10, npred=10000)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter,
                                     description=" ",
                                     epilog=textwrap.dedent("""Notes for later. """))

    ### other arguments
    parser.add_argument('-f', '--datafile', action="store", dest="datafile", required=True,
                        help="Data file with original time and flux simulations.")
    parser.add_argument('-s', '--samplefile', action="store", dest="samplefile", required=True,
                        help="Data file with sampled time and flux values.")
    parser.add_argument('-r', '--resultsfile', action="store", dest="resultsfile", required=True,
                        help="Data file with results from gp method.")
    parser.add_argument('-rs', '--resultsshape', action="store", dest="resultsshape", required=True,
                        help="list of 3 numbers for the original results file dimensions.")
    parser.add_argument('-d', '--days', action="store", dest="days", required=True,
                        help="Number of days the original data was sampled for.")
    parser.add_argument('-p', '--period', action="store", dest="trueperiod", required=False, type=float, default=None,
                        help="The true period of the lightcurve")
    parser.add_argument('-n', '--namestr', action="store", dest="namestr", required=False, default="test",
                        help="Name for plots.")
    parser.add_argument('-m', '--nmodels', action="store", dest="nmodels", required=False, type=int, default=10,
                        help="Number of models to generate for lightcurve plot.")
    parser.add_argument('-np', '--npred', action="store", dest="npred", required=False, type=int, default=1000,
                        help="Number of time stamps to generate for the lightcurve plot.")

    clargs = parser.parse_args()

    data_file = clargs.datafile
    sample_file = clargs.samplefile
    results_file = clargs.resultsfile
    results_shape = clargs.resultsshape
    days = clargs.days
    true_period = clargs.trueperiod
    name_str = clargs.namestr
    nmodels = clargs.nmodels
    npred = clargs.npred

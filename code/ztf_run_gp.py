...
def read_data(filename, whitespace=False, datadir="./"):
    ...
    flux_err = pd.to_numeric(data['sigmapsf']) #using the uncertainty from the psf, check with Lynne and Daniela that this is appropriate

    
    print(tsample.head(), fsample.head(), flux_err.head())
...
def main():
    ...
    # filename isn't really important here
    obs_files = list()

    import glob, os

    for file in glob.glob("../data/paper_plots/ztf_lightcurves/christina/*.csv"):
    ...
    for i in np.arange(len(obs_files)):
        ...
        time, flux, flux_err = read_data(obs_files[i], whitespace=False)
        ...
        with h5py.File(obs_files[i][:-4] + ".hdf5", "w") as f:
